...
shared_examples :lambda_comparator_blocking_queue do

  let(:comparator) { ->(this, that){ this.to_s.length <=> that.to_s.length } }

  it 'sorts from high to low when order is :max and a block comparator is given' do
    subject = described_class.new(order: :max, &comparator)
    %w[aaa b cccc dd eeeee].each{|item| subject.push(item) }

    expect(subject.pop).to eq 'eeeee'
    expect(subject.pop).to eq 'cccc'
    expect(subject.pop).to eq 'aaa'
    expect(subject.pop).to eq 'dd'
    expect(subject.pop).to eq 'b'
  end

  it 'sorts from high to low when order is :high and a block comparator is given' do
    subject = described_class.new(order: :high, &comparator)
    %w[aaa b cccc dd eeeee].each{|item| subject.push(item) }
  ...
  it 'sorts from low to high when order is :min and a block comparator is given' do
    subject = described_class.new(order: :min, &comparator)
    %w[aaa b cccc dd eeeee].each{|item| subject.push(item) }
  ...
  it 'sorts from low to high when order is :low and a block comparator is given' do
    subject = described_class.new(order: :low, &comparator)
    %w[aaa b cccc dd eeeee].each{|item| subject.push(item) }
  ...
  it 'sorts from high to low by default when a block comparator is given' do
...
module Concurrent
  ...
  describe MutexPriorityBlockingQueue do
    ...
    it_behaves_like :lambda_comparator_blocking_queue
  end
